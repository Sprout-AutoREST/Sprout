...
public class SproutProcessor extends AbstractProcessor {
    ...
    public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
        ...
        for (TypeElement type : classes) {
            ...
            FieldSpec idField;
            try {
                var idType = findIdType(type);
                var typeName = TypeName.get(idType);
                processingEnv.getMessager().printMessage(Diagnostic.Kind.NOTE,
                        "[Sprout] ID for " + type.getSimpleName() + " -> " + idType);

                idField = FieldSpec
                        .builder(typeName, "ID_TYPE", Modifier.PUBLIC, Modifier.STATIC)
            ...
            TypeSpec marker = TypeSpec.classBuilder(className)
            ...
            JavaFile jf = JavaFile.builder(basePackage, marker).skipJavaLangImports(true).build();
            try {
