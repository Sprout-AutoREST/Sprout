...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiError> onInvalid(MethodArgumentNotValidException ex, HttpServletRequest req) {
        ...
        return build(req, HttpStatus.BAD_REQUEST, "validation_failed", "Request body validation failed",
                errors, ex, false);
    }
    ...
    public ResponseEntity<ApiError> onConstraint(ConstraintViolationException ex, HttpServletRequest req) {
        ...
        return build(req, HttpStatus.BAD_REQUEST, "constraint_violation", "Constraint validation failed",
                errors, ex, false);
    }

    @ExceptionHandler(HttpMessageNotReadableException.class)
    public ResponseEntity<ApiError> onNotReadable(HttpMessageNotReadableException ex, HttpServletRequest req) {
        return build(req, HttpStatus.BAD_REQUEST, "malformed_json", "Malformed JSON request body",
                null, ex, false);
    }
    ...
    public ResponseEntity<ApiError> onTypeMismatch(MethodArgumentTypeMismatchException ex, HttpServletRequest req) {
        ...
        return build(req, HttpStatus.BAD_REQUEST, "type_mismatch", msg, null, ex, false);
    }
    ...
    public ResponseEntity<ApiError> onMissingParam(MissingServletRequestParameterException ex, HttpServletRequest req) {
        return build(req, HttpStatus.BAD_REQUEST, "missing_parameter",
                "Missing required parameter '%s'".formatted(ex.getParameterName()), null, ex, false);
    }

    @ExceptionHandler(HttpMediaTypeNotSupportedException.class)
    public ResponseEntity<ApiError> onUnsupported(HttpMediaTypeNotSupportedException ex, HttpServletRequest req) {
        return build(req, HttpStatus.UNSUPPORTED_MEDIA_TYPE, "unsupported_media_type",
                "Unsupported Content-Type", null, ex, false);
    }

    @ExceptionHandler(HttpMediaTypeNotAcceptableException.class)
    public ResponseEntity<ApiError> onNotAcceptable(HttpMediaTypeNotAcceptableException ex, HttpServletRequest req) {
        return build(req, HttpStatus.NOT_ACCEPTABLE, "not_acceptable", "Requested media type not acceptable",
                null, ex, false);
    }
    ...
    public ResponseEntity<ApiError> onMethodNotAllowed(
        ...
        return build(req, HttpStatus.METHOD_NOT_ALLOWED, "method_not_allowed", "HTTP method not allowed",
                null, ex, false);
    }

    @ExceptionHandler({NoSuchElementException.class, EntityNotFoundException.class})
    public ResponseEntity<ApiError> onNotFound(RuntimeException ex, HttpServletRequest req) {
        return build(req, HttpStatus.NOT_FOUND, "not_found", "Resource not found", null, ex,
                false);
    }

    @ExceptionHandler(DataIntegrityViolationException.class)
    public ResponseEntity<ApiError> onIntegrity(DataIntegrityViolationException ex, HttpServletRequest req) {
        return build(req, HttpStatus.CONFLICT, "data_integrity_violation", "Data integrity violation",
                null, ex, false);
    }

    @ExceptionHandler(Exception.class)
    public ResponseEntity<ApiError> onUnhandled(Exception ex, HttpServletRequest req) {
        return build(req, HttpStatus.INTERNAL_SERVER_ERROR, props.getInternalCode(), "Unexpected server error",
                null, ex, true);
    }
